...
impl<SendT, RecvT> MessageChannel<SendT, RecvT> {
    pub fn new(sender: impl MessageSender<SendT> + Send + Sync + 'static, receiver: impl MessageReceiver<Message=RecvT> + Send + Sync + 'static) -> Self {
        Self { sender: Box::new(sender), receiver: Box::new(receiver) }
    }
